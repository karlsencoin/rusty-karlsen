...
use karlsen_consensus_core::api::{ConsensusApi, DynConsensus};
use karlsen_core::{core::Core, debug, service::Service};
...
impl ConsensusManager {
    ...
    /// Explicit resetting of service handlers registered via `register_consensus_reset_handler`.
    ///
    /// Called internally by `StagingConsensus::commit` after a staging consensus is promoted
    /// to active. Exposed as a public method so that future flows which finalize a new pruning
    /// utxo set outside the staging-commit path can also trigger handler resets (e.g. utxoindex
    /// resync).
    ///
    /// Ported from upstream rusty-kaspa PR #827 (part 1: lib.rs refactor only).
