...
impl Consensus {
    ...
    fn pruning_samples_database_upgrade(&self) {
        ...
        let mut processed = 0;
        karlsen_core::info!("Upgrading database to include and populate the pruning samples store");
        ...
        karlsen_core::info!("Done upgrading database (populated {} entries)", processed);
    }
    ...
    fn get_current_block_color(&self, hash: Hash) -> Option<bool> {
        ...
        while let Some(Reverse(SortableBlock { hash: descendent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(descendent, sink) {
                ...
                // unreachable.
                karlsen_core::warn!("DAG topology inconsistency: {descendent} is expected to be a merging block of {hash}");
        ...
        while let Some(Reverse(SortableBlock { hash: decedent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(decedent, sink) {
                ...
                return Ok(if decedent_data.mergeset_blues.contains(&hash) {
                    ...
                    // found must also be our merging block, so hash will be either in blues or in reds, rendering this line
                    // unreachable.
                    karlsen_core::warn!("DAG topology inconsistency: {decedent} is expected to be a merging block of {hash}");
                    None
                });
            }
